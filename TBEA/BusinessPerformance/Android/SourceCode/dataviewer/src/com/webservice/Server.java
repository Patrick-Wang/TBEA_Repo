							...
							try {
							    ...
							    JSONArray qhmxBeans_Copper_JsonArray = json.getJSONArray(0);
			                      ...
			                      for (int i = 0; i < qhmxBeans_Copper_JsonArray.length(); i++) {
			                          QHMXBean qhmxBean = (QHMXBean) JsonUtil.jsonToBean(qhmxBeans_Copper_JsonArray.getJSONObject(i),
			                                  QHMXBean.class);
			                          qhmxBeans_Copper.add(qhmxBean);
			                      }
			                      for (int i = 0; i < qhmxBeans_Aluminium_JsonArray.length(); i++) {
			                          QHMXBean qhmxBean = (QHMXBean) JsonUtil.jsonToBean(qhmxBeans_Aluminium_JsonArray.getJSONObject(i),
			                                  QHMXBean.class);
