...
public class WordTemplateXmlInterpreter implements XmlInterpreter {
    ...
    final static String IMAGE_DATA_URL_REGEX = "^data:image/[a-z]+;base64,";
                    ...
                    if (!val.startsWith(WordCompileTag.MERGECOMPILE)) {
                        ...
                        if (obj != null && obj instanceof List) {
                            ...
                            if (TypeUtil.isString(obj.getClass())) {
                                ...
                                Pattern p = Pattern.compile(IMAGE_DATA_URL_REGEX);
                                Matcher m = p.matcher(sval);
                                if (m.find()) {
                                    BinaryPartAbstractImage img = DocxUtil.base642Image(pkg, sval.substring(sval.indexOf(',') + 1));
                                    DocxUtil.imageReplace(img, fdChar);
                                } else {
                                    DocxUtil.textReplace(sval, fdChar);
                                }
                            } else {
