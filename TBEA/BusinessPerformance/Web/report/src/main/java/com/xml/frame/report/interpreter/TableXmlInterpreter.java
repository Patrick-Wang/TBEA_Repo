...
public class TableXmlInterpreter implements XmlInterpreter {
	...
	protected void parseSumCol(AbstractXmlComponent component, Table tb, Element elem) throws Exception {
		...
		if (elem.hasAttribute("from")) {
			...
			for (int j = 0; j < targetCol.size(); ++j) {
				targetCol.set(j, MathUtil.sum(MathUtil.o2d(firstCol.get(j)), MathUtil.o2d(secondCol.get(j))));
			}
		}
	...
	private void parseOrderby(AbstractXmlComponent component, Table tb, Element e) throws Exception {
		...
		if (null != col) {
			...
			copies.addAll((List) tb.getValues().get(col));
	...
	private Object o2s(Object o) {
		Object r = o;
		if (null != o) {
			if (o instanceof Date) {
				r = DateUtil.day((Date) o);
			} else if (o instanceof Timestamp) {
				r = DateUtil.second((Timestamp) o);
			}
		}
		return r;
	}
	...
	protected void parseGrowthRates(AbstractXmlComponent component, Table tb, Element elem) throws Exception {
		...
		if (sj != null && tq != null && target != null) {
			...
			List<Integer> excludeRows = parserArray(XmlElWalker.elementText(list, elp, 0));
	...
	private int getTargetIndex(AbstractXmlComponent component, Element item, Table tb) throws Exception {
		ELParser elp = new ELParser(component);
	...
	Set<Integer> getAllRows(Element item, Table tb) throws Exception {
		...
		if (list.getLength() > 0) {
			List<Integer> rangeRow = idsToRows(parserArray(XmlElWalker.elementText(list, elp, 0)), tb);
		...
		if (list.getLength() > 0) {
			List<Integer> excRows = idsToRows(parserArray(XmlElWalker.elementText(list, elp, 0)), tb);
	...
	private void parseSumRow(AbstractXmlComponent component, Table tb, Element item) throws Exception {
			...
			for (int i = 0, len = tb.getValues().size(); i < len; ++i) {
				...
				for (Integer row : rows) {
					...
					if (val instanceof Double) {
						tbCol.set(targetRow, MathUtil.sum((Double) tbCol.get(targetRow), (Double) val));
					} else if (val instanceof Integer) {
						tbCol.set(targetRow, MathUtil.sum((Integer) tbCol.get(targetRow), (Integer) val));
					}
				}
			}
		} else {
	...
	private void parseMinusRow(AbstractXmlComponent component, Table tb, Element item) throws Exception {
		Integer f = XmlUtil.getIntAttr(item, "first", elp, null);
		Integer s = XmlUtil.getIntAttr(item, "second", elp, null);
		Integer t = XmlUtil.getIntAttr(item, "toRow", elp, null);

		Set<Integer> excludeCols = getExcludeCols(item);

		List tbCol = null;
		for (int i = 0, len = tb.getValues().size(); i < len; ++i) {
			if (excludeCols != null && excludeCols.contains(i)) {
				continue;
			}
			tbCol = tb.getValues().get(i);
			Object val = tbCol.get(f);
			if (val instanceof Double) {
				tbCol.set(t, MathUtil.minus((Double) val, (Double) tbCol.get(s)));
			} else if (val instanceof Integer) {
				tbCol.set(t, MathUtil.minus((Integer) val, (Integer) tbCol.get(s)));
			}
		}

	}

	private void parseMinusCol(AbstractXmlComponent component, Table tb, Element elem) throws Exception {
		ELParser elp = new ELParser(component);
	...
	private Double div(Double sub, Double base) {
		if (sub != null && base != null && (MathUtil.isNegative(sub) || MathUtil.isNegative(base)
	...
	private void parseDivRow(AbstractXmlComponent component, Table tb, Element item) throws Exception {
		...
		if (index >= 0) {
			...
			ELParser elp = new ELParser(component);
			if (list.getLength() > 0) {
				Integer subId = XmlUtil.getInt(XmlElWalker.elementText(list, elp, 0), elp, null);
			...
			if (null == subRow) {
				...
				if (list.getLength() > 0) {
					subRow = XmlUtil.getInt(XmlElWalker.elementText(list, elp, 0), elp, null);
				}
			}
			...
			if (null != subRow) {
				...
				if (list.getLength() > 0) {
					Integer baseId = XmlUtil.getInt(XmlElWalker.elementText(list, elp, 0), elp, null);
				...
				if (null == baseRow) {
					...
					if (list.getLength() > 0) {
						baseRow = XmlUtil.getInt(XmlElWalker.elementText(list, elp, 0), elp, null);
					}
				}
			}
			...
			if (subRow != null && baseRow != null) {
				...
				List<Integer> excludeCols = parserArray(XmlElWalker.elementText(list, elp, 0));

				List<Object> col = null;
				for (int i = 0; i < tb.getValues().size(); ++i) {
					if (excludeCols.indexOf(i) < 0) {
						col = tb.getValues().get(i);
						if (col.get(subRow) instanceof Double || col.get(baseRow) instanceof Double) {
							col.set(index, div((Double) col.get(subRow), (Double) col.get(baseRow)));
						}
					}
				}
			}
