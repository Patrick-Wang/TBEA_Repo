...
//				.addType(15, CellFormatter.CellType.PERCENT);
	...
	@RequestMapping(value = "hzb_zbhz_mobile.do", method = RequestMethod.GET)
	public ModelAndView getGszb_zbhz_mobile(HttpServletRequest request,
			HttpServletResponse response) {
		...
		DateSelection dateSel = new DateSelection(Calendar.getInstance(),
				true, false);
		dateSel.select(map);
		return new ModelAndView("hzb_zbhz_mobile", map);
	}
