...
AbstractCompositeConfigurator{
	...
	protected void onConfiguring(IPipe pipe, IAccumulator acc, List<Integer> zbs,
		...
		pipe.addFilter(new AccPipeFilter(acc, 0, startRow, step)
			...
			// 季度去年同期
			.addFilter(new AccPipeFilter(acc, 10, startRow, step)
				.includeCompanies(subComps))
			...
			// 去年同期
			.addFilter(new AccPipeFilter(acc, 14, startRow, step)
				.includeCompanies(subComps))
			...
			// 下季度首月预计
			.addFilter(new AccPipeFilter(acc, 16, startRow, step)
				.includeCompanies(subComps)
				)

			// 下季度次月预计
			.addFilter(new AccPipeFilter(acc, 17, startRow, step)
				.includeCompanies(subComps))

			// 下季度末月预计
			.addFilter(new AccPipeFilter(acc, 18, startRow, step)
				.includeCompanies(subComps))

			// 下季度预计合计
			.addFilter(new AccPipeFilter(acc, 19, startRow, step)
				.includeCompanies(subComps))
			...
			// 下季度年度累计
			.addFilter(new AccPipeFilter(acc, 21, startRow, step)
				.includeCompanies(subComps))
			...
			// 下季度去年同期年度累计
			.addFilter(new AccPipeFilter(acc, 23, startRow, step)
				.includeCompanies(subComps))
