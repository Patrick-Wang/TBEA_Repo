...
public class FxNhqkController {
	...
	public @ResponseBody byte[] getZBEntryUpdate(HttpServletRequest request,
		...
		List<String[]> ret =  fxNhqkService.getZb(date, dwxxId, "20012");
	...
	public @ResponseBody byte[] gethzb_companys_prediction_export(
		...
		Date date = DateSelection.getDate(request);
		List<String[]> data = fxNhqkService.getZb(date, request.getParameter("companyId"), "20021");
		JygkZzyExcelTemplate template = JygkZzyExcelTemplate.createJygkTemplate("20021");
		...
		JygkZzyFormatterHandler formatterChain = this.getFormatterChainWithHeader(
				new Integer[]{}, new Integer[]{2,3,4,5,6,7,8});
		...
		for (int i = 0; i < 2; ++i) {
			...
			for (int j = 1, jlen = data.get(i).length; j < jlen; ++j) {
				...
				if(j == 1){
					...
					formatterChain.handle(
							data.get(i + 2)[0], 
							j, template, cell, data.get(i + 2)[j]);
				}
			}
		}
