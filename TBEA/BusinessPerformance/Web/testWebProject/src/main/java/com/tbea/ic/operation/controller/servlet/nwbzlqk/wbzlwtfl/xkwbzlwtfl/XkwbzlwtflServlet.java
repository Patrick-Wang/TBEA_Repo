package com.tbea.ic.operation.controller.servlet.nwbzlqk.wbzlwtfl.xkwbzlwtfl;
...
@Controller
@RequestMapping(value = "xkwbzlwtfl")
public class XkwbzlwtflServlet {
	...
	public @ResponseBody byte[] getXknwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdWbzlwtfl(d, zts);
			}else{
				result = xknwbzlztqkService.getYdWbzlwtfl(d, zts);
			}
		}else{
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdWbzlwtfl(d, company, zts);
			}else{
				result = xknwbzlztqkService.getYdWbzlwtfl(d, company, zts);
			}
		}
