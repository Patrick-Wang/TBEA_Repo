package com.tbea.ic.operation.controller.servlet.nwbzlqk.ysazzlwt.xkysazzlwt;
...
@Controller
@RequestMapping(value = "xkysazzlwt")
public class XkysazzlwtServlet {
	...
	public @ResponseBody byte[] getXknwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdYsazzlwt(d, zts);
			}else{
				result = xknwbzlztqkService.getYdYsazzlwt(d, zts);
			}
			wis = xknwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, zts);
		}else{

			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdYsazzlwt(d, company, zts);
			}else{
				result = xknwbzlztqkService.getYdYsazzlwt(d, company, zts);
			}
			wis = xknwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, company, zts);
		}
