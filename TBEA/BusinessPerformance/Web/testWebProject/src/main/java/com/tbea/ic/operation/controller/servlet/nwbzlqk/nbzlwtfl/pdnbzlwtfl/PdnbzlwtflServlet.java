package com.tbea.ic.operation.controller.servlet.nwbzlqk.nbzlwtfl.pdnbzlwtfl;
...
@Controller
@RequestMapping(value = "pdnbzlwtfl")
public class PdnbzlwtflServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.PDCY){
			...
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdNbzlwtfl(d, zts);
			}else{
				result = pdnwbzlztqkService.getYdNbzlwtfl(d, zts);
			}
		}else{
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdNbzlwtfl(d, company, zts);
			}else{
				result = pdnwbzlztqkService.getYdNbzlwtfl(d, company, zts);
			}
		}
