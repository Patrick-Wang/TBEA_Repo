package com.tbea.ic.operation.controller.servlet.nwbzlqk.nbzlwtfl.xlnbzlwtfl;
...
@Controller
@RequestMapping(value = "xlnbzlwtfl")
public class XlnbzlwtflServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.XLCY){
			...
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdNbzlwtfl(d, zts);
			}else{
				result = xlnwbzlztqkService.getYdNbzlwtfl(d, zts); 
			}
		}else{		
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdNbzlwtfl(d, company, zts);
			}else{
				result = xlnwbzlztqkService.getYdNbzlwtfl(d, company, zts);
			}
		}
