package com.tbea.ic.operation.controller.servlet.nwbzlqk.sczzzlwt.xksczzzlwt;
...
@Controller
@RequestMapping(value = "xksczzzlwt")
public class XksczzzlqkServlet {
	...
	public @ResponseBody byte[] getXksczzzlqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdSczzzlqk(d, zts);
			}else{
				result = xknwbzlztqkService.getYdSczzzlqk(d, zts);
			}
			wis = xknwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdSczzzlqk(d, company, zts);
			}else{
				result = xknwbzlztqkService.getYdSczzzlqk(d, company, zts);
			}
			wis = xknwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, company, zts);
		}
