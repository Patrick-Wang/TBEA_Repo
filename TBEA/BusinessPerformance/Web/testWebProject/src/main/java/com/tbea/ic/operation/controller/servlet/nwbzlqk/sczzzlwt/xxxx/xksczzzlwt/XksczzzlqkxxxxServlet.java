package com.tbea.ic.operation.controller.servlet.nwbzlqk.sczzzlwt.xxxx.xksczzzlwt;
...
@Controller
@RequestMapping(value = "xksczzzlwtxxxx")
public class XksczzzlqkxxxxServlet {
	...
	public @ResponseBody byte[] getXksczzzlqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdSczzzlqkxxxx(d, zts);
			}else{
				result = xknwbzlztqkService.getYdSczzzlqkxxxx(d, zts);
			}
		}else{
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdSczzzlqkxxxx(d, company, zts);
			}else{
				result = xknwbzlztqkService.getYdSczzzlqkxxxx(d, company, zts);
			}
		}
