	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdsjzlqk(d, zts);
			}else{
				result = byqnwbzlztqkService.getYdsjzlqk(d, zts);
			}
			wis = byqnwbzlztqkService.getSjzlqkWaveItems(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdsjzlqk(d, company, zts);
			}else{
				result = byqnwbzlztqkService.getYdsjzlqk(d, company, zts);
			}
			wis = byqnwbzlztqkService.getSjzlqkWaveItems(d, yjType, company, zts);
		}
