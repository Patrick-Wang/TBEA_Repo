package com.tbea.ic.operation.controller.servlet.cwgb;
...
@Controller
@RequestMapping(value = "cwgb")
public class CwgbServlet {
	...
	public ModelAndView getShow(HttpServletRequest request,
		...
		compSel.select(map);
		return new ModelAndView("cwgb/cwgb", map);
	}
	...
	public ModelAndView getEntry(HttpServletRequest request,
		...
		CompanySelection compSel = new CompanySelection(true, COMPS);
		compSel.select(map);
		return new ModelAndView("cwgb/cwgbEntry", map);
	}
}