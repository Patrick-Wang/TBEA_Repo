...
@Controller
@RequestMapping(value = "xlgyzlqk")
public class XlgyzlwtServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.XLCY){
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdGyzlwt(d, zts);
			}else{
				result = xlnwbzlztqkService.getYdGyzlwt(d, zts); 
			}
			wis = xlnwbzlztqkService.getWaveItemsGyzlwt(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdGyzlwt(d, company, zts);
			}else{
				result = xlnwbzlztqkService.getYdGyzlwt(d, company, zts);
			}
			wis = xlnwbzlztqkService.getWaveItemsGyzlwt(d, yjType, company, zts);
		}
