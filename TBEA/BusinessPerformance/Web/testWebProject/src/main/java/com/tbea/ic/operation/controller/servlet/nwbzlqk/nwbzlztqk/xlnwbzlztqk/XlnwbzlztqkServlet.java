package com.tbea.ic.operation.controller.servlet.nwbzlqk.nwbzlztqk.xlnwbzlztqk;
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.XLCY){
			...
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdnwbzlqk(d, zts);
			}else{
				result = xlnwbzlztqkService.getYdnwbzlqk(d, zts); 
			}
			wis = xlnwbzlztqkService.getWaveItems(d, yjType, zts);
		}else{

			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdnwbzlqk(d, company, zts);
			}else{
				result = xlnwbzlztqkService.getYdnwbzlqk(d, company, zts);
			}
			wis = xlnwbzlztqkService.getWaveItems(d, yjType, company, zts);
		}
