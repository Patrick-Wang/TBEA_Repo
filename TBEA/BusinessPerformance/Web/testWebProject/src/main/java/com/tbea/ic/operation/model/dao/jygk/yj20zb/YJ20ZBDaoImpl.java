...
//	}
	...
	public List<Object[]> getEntryCompletedCompanies(Date date) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		Query q = this.getEntityManager().createQuery("select dwxx.id, yj20shzt.name from YJ20ZB where nf = :nf and yf = :yf group by dwxx.id, yj20shzt.name");
		q.setParameter("nf", cal.get(Calendar.YEAR));
		q.setParameter("yf", cal.get(Calendar.MONTH) + 1);
		return q.getResultList();
	}
...
//	}
	...
	public Double getZb(Integer zbId, Date date, List<Integer> companies) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		Query q = this.getEntityManager().createQuery("select sum(yj20z) from YJ20ZB where zbxx.id = :id and nf = :nf and yf = :yf and dwxx.id in :comp");
