package com.tbea.ic.operation.controller.servlet.nwbzlqk.wbzlwtfl.pdwbzlwtfl;
...
@Controller
@RequestMapping(value = "pdwbzlwtfl")
public class PdwbzlwtflServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.PDCY){
			...
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdWbzlwtfl(d, zts);
			}else{
				result = pdnwbzlztqkService.getYdWbzlwtfl(d, zts);
			}
		}else{
			
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdWbzlwtfl(d, company, zts);
			}else{
				result = pdnwbzlztqkService.getYdWbzlwtfl(d, company, zts);
			}
		}
