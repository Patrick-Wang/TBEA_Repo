	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (all){
			...
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdYsazzlwt(d);
			}else{
				result = xlnwbzlztqkService.getYdYsazzlwt(d); 
			}
			wis = xlnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType);
		}else{
			CompanyType comp = CompanySelection.getCompany(request);
			Company company = companyManager.getVirtualCYOrg().getCompany(comp);
			
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdYsazzlwt(d, company);
			}else{
				result = xlnwbzlztqkService.getYdYsazzlwt(d, company);
			}
			wis = xlnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, company);
		}
