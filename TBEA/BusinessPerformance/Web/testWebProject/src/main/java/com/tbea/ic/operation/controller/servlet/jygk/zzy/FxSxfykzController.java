...
@Controller
@RequestMapping(value = "fxsxfykz")
public class FxSxfykzController {
	...
	public @ResponseBody byte[] export(
		...
		String year = request.getParameter("year");
		String month = request.getParameter("month");
		List<String[]> data =  fxSxfykzService.getViewDataList(dwxxId,year,month);
		String fileNameAndSheetName = dwxxname;
		fileNameAndSheetName += request.getParameter("year") + "年" + request.getParameter("month") + "月三项费用管控";		
		JygkZzyExcelTemplate template = JygkZzyExcelTemplate.createJygkTemplate("20020");		
		HSSFWorkbook workbook = template.getWorkbook();
		HSSFSheet sheet = workbook.getSheetAt(0);

		JygkZzyFormatterHandler formatterChain = this.getFormatterChainWithHeader(
				new Integer[]{3,9,10,12,13,15,16,18}, new Integer[]{1, 2, 4, 6, 7,8,11,14,17});
