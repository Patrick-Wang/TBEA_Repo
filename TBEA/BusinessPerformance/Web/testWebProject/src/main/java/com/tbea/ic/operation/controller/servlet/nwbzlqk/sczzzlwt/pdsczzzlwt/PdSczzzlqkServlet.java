	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.PDCY){
			...
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdSczzzlqk(d, zts);
			}else{
				result = pdnwbzlztqkService.getYdSczzzlqk(d, zts);
			}
			wis = pdnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdSczzzlqk(d, company, zts);
			}else{
				result = pdnwbzlztqkService.getYdSczzzlqk(d, company, zts);
			}
			wis = pdnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, company, zts);
		}
