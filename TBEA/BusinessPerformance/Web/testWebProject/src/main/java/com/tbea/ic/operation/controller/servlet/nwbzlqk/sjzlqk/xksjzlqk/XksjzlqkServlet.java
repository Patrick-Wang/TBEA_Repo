package com.tbea.ic.operation.controller.servlet.nwbzlqk.sjzlqk.xksjzlqk;
...
@Controller
@RequestMapping(value = "xksjzlqk")
public class XksjzlqkServlet {
	...
	public @ResponseBody byte[] getXknwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdsjzlqk(d, zts);
			}else{
				result = xknwbzlztqkService.getYdsjzlqk(d, zts);
			}
			wis = xknwbzlztqkService.getSjzlqkWaveItems(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdsjzlqk(d, company, zts);
			}else{
				result = xknwbzlztqkService.getYdsjzlqk(d, company, zts);
			}
			wis = xknwbzlztqkService.getSjzlqkWaveItems(d, yjType, company, zts);
		}
