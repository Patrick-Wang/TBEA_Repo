package com.tbea.ic.operation.controller.servlet.nwbzlqk.sczzzlwt.xxxx.byqsczzzlwt;
...
@Controller
@RequestMapping(value = "byqsczzzlwtxxxx")
public class ByqsczzzlqkxxxxServlet {
	...
	public @ResponseBody byte[] getByqsczzzlqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdSczzzlqkxxxx(d, zts);
			}else{
				result = byqnwbzlztqkService.getYdSczzzlqkxxxx(d, zts);
			}
		}else{
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdSczzzlqkxxxx(d, company, zts);
			}else{
				result = byqnwbzlztqkService.getYdSczzzlqkxxxx(d, company, zts);
			}
		}
