...
public class BYQCBServiceImpl implements BYQCBService {
	...
	private void filltbqk(String[][] tb, int row, CBBYQTBDD byqtbcb,
		...
		tb[tb.length - 1][0] = valueOf(tb[tb.length - 1][0]) + byqtbcb.getCz()
				+ "";
		tb[tb.length - 1][1] = valueOf(tb[tb.length - 1][1]) + byqtbcb.getCz()
				- tbzccb + "";
		tb[tb.length - 1][2] = valueOf(tb[tb.length - 1][1])
				/ valueOf(tb[tb.length - 1][0]) + "";
		tb[tb.length - 1][3] = valueOf(tb[tb.length - 1][3])
				+ byqtbcb.getGgdj() + "";
		tb[tb.length - 1][4] = valueOf(tb[tb.length - 1][4])
				+ byqtbcb.getGgyl() + "";
		tb[tb.length - 1][5] = valueOf(tb[tb.length - 1][5])
				+ byqtbcb.getDjtdj() + "";
		tb[tb.length - 1][6] = valueOf(tb[tb.length - 1][6])
				+ byqtbcb.getDjtyl() + "";
		tb[tb.length - 1][7] = "0";// 电解铜费用
		tb[tb.length - 1][8] = valueOf(tb[tb.length - 1][8])
				+ byqtbcb.getZbdj() + "";
		tb[tb.length - 1][9] = valueOf(tb[tb.length - 1][9])
				+ byqtbcb.getZbyl() + "";
		tb[tb.length - 1][10] = valueOf(tb[tb.length - 1][10])
				+ byqtbcb.getByqydj() + "";
		tb[tb.length - 1][11] = valueOf(tb[tb.length - 1][11])
				+ byqtbcb.getByqyyl() + "";
		tb[tb.length - 1][12] = valueOf(tb[tb.length - 1][12])
				+ byqtbcb.getGcdj() + "";
		tb[tb.length - 1][13] = valueOf(tb[tb.length - 1][13])
				+ byqtbcb.getGcyl() + "";
	}
