package com.tbea.ic.operation.controller.servlet.nwbzlqk.wbzlwtfl.byqwbzlwtfl;
...
@Controller
@RequestMapping(value = "byqwbzlwtfl")
public class ByqwbzlwtflServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdWbzlwtfl(d, zts);
			}else{
				result = byqnwbzlztqkService.getYdWbzlwtfl(d, zts);
			}
		}else{
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdWbzlwtfl(d, company, zts);
			}else{
				result = byqnwbzlztqkService.getYdWbzlwtfl(d, company, zts);
			}
		}
