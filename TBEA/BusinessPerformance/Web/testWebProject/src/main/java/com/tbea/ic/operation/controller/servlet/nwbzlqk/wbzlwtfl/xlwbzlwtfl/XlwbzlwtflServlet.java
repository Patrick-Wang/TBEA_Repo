	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.XLCY){
			...
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdWbzlwtfl(d, zts);
			}else{
				result = xlnwbzlztqkService.getYdWbzlwtfl(d, zts); 
			}
		}else{

			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdWbzlwtfl(d, company, zts);
			}else{
				result = xlnwbzlztqkService.getYdWbzlwtfl(d, company, zts);
			}
		}
