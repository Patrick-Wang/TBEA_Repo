...
public class FxJkcbZtnhqkController {
	...
	public ModelAndView getZBBYQ(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		map.put("bg", fxJkcbZtnhqkService.getCksjBgList());
	...
	public ModelAndView getZBXL(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		//给页面返回报表ID和名称
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("bg", fxJkcbZtnhqkService.getCksjBgList());
	...
	public @ResponseBody byte[] getZBEntry(HttpServletRequest request,
		...
		List<Map> comps = fxJkcbZtnhqkService.getBgCompanies(request.getParameter("entryType"));
		map.put("comps", comps);
	...
	public @ResponseBody byte[] getZBEntryUpdate(HttpServletRequest request,
		...
		List<String[]> ret =  fxJkcbZtnhqkService.getZb(date, request.getParameter("companyId"), request.getParameter("entryType"));
	...
	@RequestMapping(value = "fx_jkcb_ztnhqk_export.do")
	public @ResponseBody byte[] gethzb_companys_prediction_export(
		...
		Object compname = "";
		List<Map> comps = fxJkcbZtnhqkService.getBgCompanies(entryType);
		for(int i = 0; i < comps.size(); i++){
			if(request.getParameter("companyId").equals(comps.get(i).get("id"))){
				compname = comps.get(i).get("value");
			}
		}
		List<String[]> data = fxJkcbZtnhqkService.getZb(date, request.getParameter("companyId"), request.getParameter("entryType"));
		...
		fileNameAndSheetName += request.getParameter("year") + "年" + request.getParameter("month") + "月整体能耗情况（" + ("20009".equals(entryType) ? "变压器" : "线缆") + "）";
