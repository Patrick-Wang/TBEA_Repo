	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (all){
			...
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdnwbzlqk(d);
			}else{
				result = pdnwbzlztqkService.getYdnwbzlqk(d);
			}
			wis = pdnwbzlztqkService.getWaveItems(d, yjType);
		}else{
			CompanyType comp = CompanySelection.getCompany(request);
			Company company = companyManager.getVirtualCYOrg().getCompany(comp);
			
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdnwbzlqk(d, company);
			}else{
				result = pdnwbzlztqkService.getYdnwbzlqk(d, company);
			}
			wis = pdnwbzlztqkService.getWaveItems(d, yjType, company);
		}
