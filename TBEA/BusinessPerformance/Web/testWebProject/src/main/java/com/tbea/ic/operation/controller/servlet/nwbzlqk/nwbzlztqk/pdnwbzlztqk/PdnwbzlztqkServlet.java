package com.tbea.ic.operation.controller.servlet.nwbzlqk.nwbzlztqk.pdnwbzlztqk;
...
@Controller
@RequestMapping(value = "pdnwbzlztqk")
public class PdnwbzlztqkServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.PDCY){
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdnwbzlqk(d, zts);
			}else{
				result = pdnwbzlztqkService.getYdnwbzlqk(d, zts);
			}
			wis = pdnwbzlztqkService.getWaveItems(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdnwbzlqk(d, company, zts);
			}else{
				result = pdnwbzlztqkService.getYdnwbzlqk(d, company, zts);
			}
			wis = pdnwbzlztqkService.getWaveItems(d, yjType, company, zts);
		}
