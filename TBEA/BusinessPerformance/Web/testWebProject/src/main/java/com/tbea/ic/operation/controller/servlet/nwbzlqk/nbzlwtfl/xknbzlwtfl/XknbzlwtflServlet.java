package com.tbea.ic.operation.controller.servlet.nwbzlqk.nbzlwtfl.xknbzlwtfl;
...
@Controller
@RequestMapping(value = "xknbzlwtfl")
public class XknbzlwtflServlet {
	...
	public @ResponseBody byte[] getXknwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdNbzlwtfl(d, zts);
			}else{
				result = xknwbzlztqkService.getYdNbzlwtfl(d, zts);
			}
		}else{
			if (yjType == YDJDType.JD){
				result = xknwbzlztqkService.getJdNbzlwtfl(d, company, zts);
			}else{
				result = xknwbzlztqkService.getYdNbzlwtfl(d, company, zts);
			}
		}
