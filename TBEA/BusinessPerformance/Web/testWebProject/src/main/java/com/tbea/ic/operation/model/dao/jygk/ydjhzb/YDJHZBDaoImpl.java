...
//	}
	...
	public List<Object[]> getEntryCompletedCompanies(Date date) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		Query q = this.getEntityManager().createQuery("select dwxx.id , ydjhshzt.name from YDJHZB where nf = :nf and yf = :yf group by dwxx.id, ydjhshzt.name ");
		q.setParameter("nf", cal.get(Calendar.YEAR));
		q.setParameter("yf", cal.get(Calendar.MONTH) + 1);
		return q.getResultList();
	}
...
//	}
	...
	public List<Integer> getApprovedCompletedCompanies(Date date) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		Query q = this.getEntityManager().createQuery("select dwxx.id from YDJHZB where nf = :nf and yf = :yf and ydjhshzt.id = 1 group by dwxx.id");
		q.setParameter("nf", cal.get(Calendar.YEAR));
		q.setParameter("yf", cal.get(Calendar.MONTH) + 1);
		return q.getResultList();
	}
	...
	public Double getZb(Integer zbId, Date date, List<Integer> companies) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		Query q = this.getEntityManager().createQuery("select sum(ydjhz) from YDJHZB where zbxx.id = :id and nf = :nf and yf = :yf and dwxx.id in :comp");
