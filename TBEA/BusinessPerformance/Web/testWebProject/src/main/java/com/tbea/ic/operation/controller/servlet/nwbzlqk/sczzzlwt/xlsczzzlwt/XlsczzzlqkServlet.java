	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.XLCY){
			...
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdSczzzlqk(d, zts);
			}else{
				result = xlnwbzlztqkService.getYdSczzzlqk(d, zts); 
			}
			wis = xlnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdSczzzlqk(d, company, zts);
			}else{
				result = xlnwbzlztqkService.getYdSczzzlqk(d, company, zts);
			}
			wis = xlnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, company, zts);
		}
