package com.tbea.ic.operation.controller.servlet.nwbzlqk.sczzzlwt.xlsczzzlwt;
...
@Controller
@RequestMapping(value = "xlsczzzlwt")
public class XlsczzzlqkServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (all){
			...
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdSczzzlqk(d);
			}else{
				result = xlnwbzlztqkService.getYdSczzzlqk(d); 
			}
			wis = xlnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType);
		}else{
			CompanyType comp = CompanySelection.getCompany(request);
			Company company = companyManager.getVirtualCYOrg().getCompany(comp);
			
			if (yjType == YDJDType.JD){
				result = xlnwbzlztqkService.getJdSczzzlqk(d, company);
			}else{
				result = xlnwbzlztqkService.getYdSczzzlqk(d, company);
			}
			wis = xlnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, company);
		}
