	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (all){
			...
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdsjzlqk(d);
			}else{
				result = pdnwbzlztqkService.getYdsjzlqk(d);
			}
			wis = pdnwbzlztqkService.getSjzlqkWaveItems(d, yjType);
		}else{
			CompanyType comp = CompanySelection.getCompany(request);
			Company company = companyManager.getVirtualCYOrg().getCompany(comp);
			
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdSjzlqk(d, company);
			}else{
				result = pdnwbzlztqkService.getYdSjzlqk(d, company);
			}
			wis = pdnwbzlztqkService.getSjzlqkWaveItems(d, yjType, company);
		}
