package com.tbea.ic.operation.controller.servlet.nwbzlqk.sjzlqk.pdsjzlqk;
...
@Controller
@RequestMapping(value = "pdsjzlqk")
public class PdsjzlqkServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.PDCY){
			...
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdsjzlqk(d, zts);
			}else{
				result = pdnwbzlztqkService.getYdsjzlqk(d, zts);
			}
			wis = pdnwbzlztqkService.getSjzlqkWaveItems(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdSjzlqk(d, company, zts);
			}else{
				result = pdnwbzlztqkService.getYdSjzlqk(d, company, zts);
			}
			wis = pdnwbzlztqkService.getSjzlqkWaveItems(d, yjType, company, zts);
		}
