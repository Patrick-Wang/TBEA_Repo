package com.tbea.ic.operation.controller.servlet.nwbzlqk.ysazzlwt.pdysazzlwt;
...
@Controller
@RequestMapping(value = "pdysazzlwt")
public class PdysazzlwtServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.PDCY){
			...
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdYsazzlwt(d, zts);
			}else{
				result = pdnwbzlztqkService.getYdYsazzlwt(d, zts);
			}
			wis = pdnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = pdnwbzlztqkService.getJdYsazzlwt(d, company, zts);
			}else{
				result = pdnwbzlztqkService.getYdYsazzlwt(d, company, zts);
			}
			wis = pdnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, company, zts);
		}
