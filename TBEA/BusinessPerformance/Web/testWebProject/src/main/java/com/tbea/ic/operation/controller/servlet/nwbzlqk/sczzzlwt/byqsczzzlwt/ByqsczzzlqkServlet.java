	...
	public @ResponseBody byte[] getByqsczzzlqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdSczzzlqk(d, zts);
			}else{
				result = byqnwbzlztqkService.getYdSczzzlqk(d, zts);
			}
			wis = byqnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, zts);
		}else{
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdSczzzlqk(d, company, zts);
			}else{
				result = byqnwbzlztqkService.getYdSczzzlqk(d, company, zts);
			}
			wis = byqnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, company, zts);
		}
