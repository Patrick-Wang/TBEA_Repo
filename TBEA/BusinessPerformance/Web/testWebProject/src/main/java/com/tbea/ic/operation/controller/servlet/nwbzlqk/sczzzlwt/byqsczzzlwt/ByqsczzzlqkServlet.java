package com.tbea.ic.operation.controller.servlet.nwbzlqk.sczzzlwt.byqsczzzlwt;
...
@Controller
@RequestMapping(value = "byqsczzzlwt")
public class ByqsczzzlqkServlet {
	...
	public @ResponseBody byte[] getByqsczzzlqk(HttpServletRequest request,
		...
		if (all){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdSczzzlqk(d);
			}else{
				result = byqnwbzlztqkService.getYdSczzzlqk(d);
			}
			wis = byqnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType);
		}else{
			CompanyType comp = CompanySelection.getCompany(request);
			Company company = companyManager.getVirtualCYOrg().getCompany(comp);
			
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdSczzzlqk(d, company);
			}else{
				result = byqnwbzlztqkService.getYdSczzzlqk(d, company);
			}
			wis = byqnwbzlztqkService.getWaveItemsSczzzlqk(d, yjType, company);
		}
