package com.tbea.ic.operation.controller.servlet.cpzlqkyd;
...
@Controller
@RequestMapping(value = "cpzlqkyd")
public class CpzlqkydServlet {
	...
	public ModelAndView getShow(HttpServletRequest request,
		...
		compSel.select(map);
		return new ModelAndView("cpzlqkyd/cpzlqkyd", map);
	}
	...
	public ModelAndView getEntry(HttpServletRequest request,
		...
		CompanySelection compSel = new CompanySelection(true, COMPS);
		compSel.select(map);
		return new ModelAndView("cpzlqkyd/cpzlqkydEntry", map);
	}
}