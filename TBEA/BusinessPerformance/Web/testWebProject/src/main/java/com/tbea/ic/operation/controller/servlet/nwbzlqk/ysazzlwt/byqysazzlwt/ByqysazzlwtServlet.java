	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (all){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdYsazzlwt(d);
			}else{
				result = byqnwbzlztqkService.getYdYsazzlwt(d);
			}
			wis = byqnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType);
		}else{
			CompanyType comp = CompanySelection.getCompany(request);
			Company company = companyManager.getVirtualCYOrg().getCompany(comp);
			
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdYsazzlwt(d, company);
			}else{
				result = byqnwbzlztqkService.getYdYsazzlwt(d, company);
			}
			wis = byqnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, company);
		}
