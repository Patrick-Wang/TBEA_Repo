package com.tbea.ic.operation.controller.servlet.nwbzlqk.ysazzlwt.byqysazzlwt;
...
@Controller
@RequestMapping(value = "byqysazzlwt")
public class ByqysazzlwtServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdYsazzlwt(d, zts);
			}else{
				result = byqnwbzlztqkService.getYdYsazzlwt(d, zts);
			}
			wis = byqnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, zts);
		}else{

			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdYsazzlwt(d, company, zts);
			}else{
				result = byqnwbzlztqkService.getYdYsazzlwt(d, company, zts);
			}
			wis = byqnwbzlztqkService.getWaveItemsYsazzlwt(d, yjType, company, zts);
		}
