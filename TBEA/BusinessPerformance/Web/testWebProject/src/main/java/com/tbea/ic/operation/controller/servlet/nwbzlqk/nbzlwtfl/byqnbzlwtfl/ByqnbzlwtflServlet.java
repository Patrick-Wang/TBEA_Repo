package com.tbea.ic.operation.controller.servlet.nwbzlqk.nbzlwtfl.byqnbzlwtfl;
...
@Controller
@RequestMapping(value = "byqnbzlwtfl")
public class ByqnbzlwtflServlet {
	...
	public @ResponseBody byte[] getByqnwbzlztqk(HttpServletRequest request,
		...
		if (comp == CompanyType.BYQCY){
			...
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdNbzlwtfl(d, zts);
			}else{
				result = byqnwbzlztqkService.getYdNbzlwtfl(d, zts);
			}
		}else{
			if (yjType == YDJDType.JD){
				result = byqnwbzlztqkService.getJdNbzlwtfl(d, company, zts);
			}else{
				result = byqnwbzlztqkService.getYdNbzlwtfl(d, company, zts);
			}
		}
