    ...
    export class View {
        ...
        public init(tableId: string, dateId: string, month: number, year: number): void {
          ...
          // $("#date1").append("<input type=\"text\" id=\"dateInput\" class=\"Wdate\"/>");
            ...
            //$("#dateInput")[0].onfocus = WdatePicker({skin:'whyGreen',minDate:(year - 3) + '-1-1',maxDate:year + '-' + month+'-1'});
            //$("#dateInput").click(()=>{
            //    WdatePicker({el:'dateInput', skin:'twoer',minDate:'2006-09-10',maxDate:'2008-12-20'});
            //});
            this.updateTable();
            this.updateUI();
...
//                    cellEdit: true,
                    ...
                    rowNum: 200,
                    onSelectRow:(id:any) => {
                        var curRowData =  $("#" + name).jqGrid('getRowData', id);
                        $($("#" + name).jqGrid('getGridRowById',id)).removeClass("ui-state-highlight");
                        $("#" + name).jqGrid("setSelection", id,false);
                    },
                    autoScroll: true
                }));
