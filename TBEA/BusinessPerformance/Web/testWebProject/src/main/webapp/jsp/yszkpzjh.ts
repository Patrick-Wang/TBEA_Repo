...
module yszkpzjh {
    ...
    class JQGridAssistantFactory {
        ...
        public static createYSPZ1(gridName): JQTable.JQGridAssistant{
        ...
        public static createYSPZ3(gridName: string): JQTable.JQGridAssistant{
            return new JQTable.JQGridAssistant([
                    new JQTable.Node("上月末账面<br/>应收余额", "symzbysye", true, JQTable.TextAlign.Right, 100)
                ], gridName);
	    }

        public static createYSPZ4(gridName): JQTable.JQGridAssistant{
            return new JQTable.JQGridAssistant([
    ...
    export class View {
        ...
        private mDataSet : Util.Ajax = new Util.Ajax("yszkpzjh_update.do");
        ...
        private refreshTable(listName : string, refreshTags : boolean[], list : number[]) : void {
			for (var i = 0; i < refreshTags.length; i++) {
				if (refreshTags[i]) {
					if (!(list[i] >= 0)) {
						$("#" + listName + (i + 1) + "_jqgrid_1234").jqGrid('saveCell', '1',
								list[i]);
						list[i] = -1;
					}
            ...
            $("#" + name1).jqGrid(t1Assist.decorate(
                {
                    ...
                    onSelectCell: (id, nm, tmp, iRow, iCol) => {
                        ...
                        this.refreshTable(listName, refreshTags,
                            listCurrentColum);
                        t1Assist.selected(iRow, iCol);
                    },
                    gridComplete: () => {
                        $("#gbox_" + name1).css("border-bottom",
                            "0px solid #a6c9e2");
            ...
            $("#" + name2).jqGrid(t2Assist.decorate(
                {
                    ...
                    gridComplete: () => {
                        $("#gbox_" + name2).css("border-left",
                            "0px solid #a6c9e2");
                        $("#gbox_" + name2).css("border-bottom",
                            "0px solid #a6c9e2");
