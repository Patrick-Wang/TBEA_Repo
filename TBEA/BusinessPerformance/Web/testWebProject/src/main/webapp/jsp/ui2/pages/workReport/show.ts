...
module framework.templates.dateReport {
    ...
    class ItemShowView extends framework.templates.singleDateReport.ShowView{
        ...
        onUpdateChartsData(){
            ...
            //收入变化趋势
            var bhqs_srChart:any = echarts.init(document.getElementById("bhqs_sr"));
            var bhqs_sroption = {
                animation: false,
                title: {
                    text: '收入变化趋势' ,
            ...
            //销售利润率变化趋势
            let bhqs_xslrChart : any = echarts.init(document.getElementById("bhqs_xslr"));
            var bhqs_xslroption = {
                animation: false,
                title: {
                    text: '销售利润率变化趋势' ,
        ...
        update (date:Util.Date){
            if (!this.firstUpdate){
                this.mDataUrl.get(this.getParams(date))
                    .then((data:any) => {
                        this.resp = data;
                        this.onUpdateChartsData();
                        this.onUpdateFormData();
                        this.mAjaxUpdate.post($.extend(this.chartsData, this.getParams(date))).then((jsonData)=>{
