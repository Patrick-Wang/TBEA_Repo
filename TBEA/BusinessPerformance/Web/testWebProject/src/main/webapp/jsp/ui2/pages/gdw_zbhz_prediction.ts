...
module gdw_zbhz_prediciton{
  ...
  enum FirstMonthZb{
    ...
    export class SimpleView implements Endpoint {
        ...
        private mDataSet:Util.Ajax = new Util.Ajax("/BusinessManagement/ydzb/gdw_zbhz_prediction_update.do");
        ...
        public exportExcel() {
            $("#exportExcel")[0].action = "/BusinessManagement/ydzb/gdw_zbhz_prediction_export.do?" + Util.Ajax.toUrlParam(
                    $.extend(this.getDate(), {top5index:$("#grid-type").val(), zbName:$("#grid-type  option:selected").text()}));
