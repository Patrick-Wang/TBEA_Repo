...
module Util {
    ...
    class RelationFormual implements IFormula {
        grid: any;
        rel: Relation;
        ...
        update(editCell: any, col:any): boolean {
            if (col == this.rel.ids[0]) {
                let data = this.grid.getCell(editCell, this.rel.ids[0]);
                for (let i = 0; i < this.rel.values.length; ++i){
                    if (this.rel.values[i][0] == data) {
                        for (let j = 1; j < this.rel.ids.length; ++j) {
                            let dataDest = this.grid.getCell(editCell, this.rel.ids[j]);
                            if (!dataDest) {
    ...
    export function createTable(gridName: string, gridCtrl: Util.GridCtrl): JQTable.JQGridAssistant {
        ...
        if (gridCtrl.relations) {
            for (let i = 0; i < gridCtrl.relations.length; ++i) {
