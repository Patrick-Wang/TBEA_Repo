...
--特变电工债券发行明细统计表
IF EXISTS ( SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'financing_bonds')
DROP TABLE financing_bonds
CREATE TABLE [dbo].[financing_bonds](
	[id] [int] IDENTITY(1,1) NOT NULL,
	bondType varchar(100), --债券品种
	beginningDate date, --发行时间
	currency varchar(50), --发行币种
	loanAmount numeric(18, 4), --发行金额
	loanRate numeric(18, 4), --发行利率
	floatingRatio numeric(18, 4), --利率浮动
