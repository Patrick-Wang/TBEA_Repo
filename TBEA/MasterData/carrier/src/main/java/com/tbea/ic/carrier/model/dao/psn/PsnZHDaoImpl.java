...
@Repository
@Transactional("transactionManager_psn_zh")
public class PsnZHDaoImpl implements PsnZHDao{

	@PersistenceContext(unitName = "localDB_psn_zh")
	...
	public String getPsnNoByID(String id) {
		...
		Query q = manager.createQuery("from PsnZH where iden = :id");
	...
	public List<Psn> getPsns(int pageIndex) {
		Query q = manager.createQuery("from PsnZH");
	...
	public int getPsnPagesCount(){
		Query q = manager.createQuery("select count(*) from PsnZH");
